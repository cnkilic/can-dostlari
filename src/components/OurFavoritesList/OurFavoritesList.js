...
const OurFavoritesList = () => {
  ...
  return (
    <div className="max-w-full relative w-full h-[250px]">
      ...
      >
        {favoriteCollectionList.map((favoriteCollection,index) => (
          <FavoriteCollection key={index} favoriteCollection={favoriteCollection} />
        ))}
      </div>
...
function FavoriteCollection({favoriteCollection}) {
  return (
    <Link href={favoriteCollection.link} className="relative h-full rounded-lg text-center shadow-lg hover:shadow-xl flex flex-col  bg-white">
      <Image
